...
### v1.0 (2025-11-20)
- **初始穩定版本**
- CPU/GPU/NPU/記憶體/網路/磁碟監控
- 多模式支援：本地、遠端 Linux（SSH）、Android（ADB）
- CPU/GPU 頻率控制（本地和 SSH 模式）
- PyQt5 圖形介面與即時圖表
- CLI 模式用於無 GUI / Server 環境
- SQLite 資料庫記錄
- 匯出為 CSV/JSON/HTML 格式
- 包含 13+ 個圖表的互動式 HTML 報告
- **錯誤修正**：
  - 修正 SSH 中斷率顯示全為零
  - 修正報告中 CPU 功耗下降至零
  - 修正 CLI 中 SSH 頻率控制支援
  - 修正匯出中的模板變數替換
- 全面測試套件（152 個單元測試，40% 覆蓋率）
- Debian 軟體套件支援
- 自動清理舊監控資料（>3 天）
